...
const ICON_CHECK = `<svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="#4ade80" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="9 12 11 14 15 10"/></svg>`;
const ICON_X = `<svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="#f87171" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`;
const PAGE_STYLE = `*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#0a0a0f;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:2rem}.card{background:#13131a;border:1px solid #ffffff18;border-radius:16px;padding:2.5rem;text-align:center;max-width:420px;width:100%}.icon{margin-bottom:1.25rem}h1{font-size:1.5rem;font-weight:700;margin-bottom:.5rem}p{color:#a1a1aa;line-height:1.6;margin-bottom:1rem}small{color:#52525b;font-size:.75rem;display:block;margin-bottom:1.5rem}a.btn{display:inline-block;background:#7c3aed;color:#fff;font-weight:700;padding:.625rem 1.5rem;border-radius:8px;text-decoration:none;font-size:.9rem}a.btn:hover{background:#6d28d9}`;

const PAGE = {
  success: (invoice) => `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Payment Successful</title><style>${PAGE_STYLE}.h1c{color:#4ade80}</style></head><body><div class="card"><div class="icon">${ICON_CHECK}</div><h1 class="h1c">Payment Successful</h1><p>Your payment was confirmed. Check your email for delivery. It should arrive within a few minutes.</p><small>Order ref: ${invoice}</small>${STORE_URL ? `<a class="btn" href="${STORE_URL}">Back to store</a>` : ""}</div></body></html>`,
  cancelled: () => `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Payment Cancelled</title><style>${PAGE_STYLE}.h1c{color:#f87171}</style></head><body><div class="card"><div class="icon">${ICON_X}</div><h1 class="h1c">Payment Cancelled</h1><p>Your payment was not completed.</p>${STORE_URL ? `<a class="btn" href="${STORE_URL}">Back to store</a>` : ""}</div></body></html>`,
};
